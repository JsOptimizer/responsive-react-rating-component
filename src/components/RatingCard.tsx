...
const RatingCard = ({setRating,setIsSubmit}:TRatingCard) => {
  return (
    <article className="bg-secondary p-8 rounded-3xl space-y-8">
      ...
      <ul className="w-full flex justify-between">
        {
          [1, 2, 3, 4,5].map((rate)=>(
            <li key={rate}><button onClick={()=>setRating(()=>rate)} className="bg-secondary-900 bg-opacity-20 text-neutral-500 rounded-full w-12 h-12 active:bg-primary hover:bg-neutral-50 hover:text-neutral  active:text-neutral ">{rate}</button></li>
          ))
        }
      </ul>
